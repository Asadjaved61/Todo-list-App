...
const Todo: React.FC<TodoProps> = ({ todo, index, updateTodoStatus, removeTodo }) => {
    ...
    return (
        <li className="todo center-element" style={{ background: todo.isCompleted ? '#999' : 'white' }}>
            { edit ? <input type="text" defaultValue={todo.text} className="input input-todoText" onChange={updateTodoText} autoFocus/> : 
            <span className="todo-text" style={{ textDecoration: todo.isCompleted ? 'line-through': '' }}>{todo.text}</span> }
